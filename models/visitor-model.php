...
class VisitorModel extends Model
{
	...
	final public function generateStrongCaptchaString(int $length): string
	{
		// These letters of Juliamo are misdetected, this is why at least one must be present
		$misdetects   = mb_str_split('bfgwBFGW');
		$characters   = mb_str_split('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ');
		$randomString = '';
		
		for ($i = 0; $i < 1; $i++)
		{
			$randomIndex  = random_int(0, count($misdetects) - 1);
			$randomString = $randomString.$misdetects[$randomIndex];
		}
		
		for ($i = 1; $i < $length; $i++)
		{
			$randomIndex  = random_int(0, count($characters) - 1);
	...
	final public function generateWeakCaptchaString(int $length): string
	{
		// These letters of Juliamo are similar to other fonts
		$characters   = mb_str_split('cdehijlkmnopqstxyCDEHIJLKMNOPQSTXY');
		$randomString = '';
	...
	final protected function generateNoise(GDImage $image, int $minIntensity, int $maxIntensity, int $alpha): void
	{
		$width  = imagesx($image);
	...
	final public function generateBase64Captcha(string $string, int $widthPx = 192, int $heightPx = 48): string
	{
		// Important! All constants are calculated empirically for the Juliamo font
		// Intensity and Transparence for generated noise are also chosen empirically
		
		$image = imagecreatetruecolor($widthPx, $heightPx);
		$this->generateNoise($image, 64, 192, 0);
		...
		$textColors = 
		[
			//imagecolorallocatealpha($image, 000, 000, 000, 000),
			//imagecolorallocatealpha($image, 000, 000, 255, 000),
			imagecolorallocatealpha($image, 000, 255, 000, 000),
			//imagecolorallocatealpha($image, 000, 255, 255, 000),
			//imagecolorallocatealpha($image, 255, 000, 000, 000),
			imagecolorallocatealpha($image, 255, 000, 255, 000),
			//imagecolorallocatealpha($image, 255, 255, 000, 000),
			//imagecolorallocatealpha($image, 255, 255, 255, 000)
		];
		...
		$xStart  = ($widthPx / ($count + 1)) - $heightPx * 192 / 418 / 2;
		$xOffset = ($widthPx / ($count + 1));
		$yStart  = $heightPx * 5 / 6;
		$yOffset = 0;
		...
		for ($i = 0; $i < $count; $i++)
		{
			...
			$color = random_int(0, count($textColors) - 1);
			
			imagettftext($image, $fontSize, $angle, $x, $y, $textColors[$color], $fontPath, $string[$i]);
		}
		
		$this->generateNoise($image, 64, 192, 96);
