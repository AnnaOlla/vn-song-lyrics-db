...
namespace Localization\HomePage
{
	...
	const DescriptionFour  = 'Этот сайт представляет собой вики, где каждый зарегистрированный пользователь может внести вклад.';
	...
	const Search                = 'Поиск';
	const SearchAll             = 'Всё';
	const SearchGameList        = 'Игры';
	const SearchAlbumList       = 'Альбомы';
	const SearchArtistList      = 'Исполнители';
	const SearchCharacterList   = 'Персонажи';
	const SearchSongList        = 'Песни';
	const SearchTranslationList = 'Переводы';
}
...
namespace Localization\Controls
{
	...
	const ChooseFile   = 'Выбрать файл (максимальный размер: 512 MiB) …';
	const FileTooBig   = 'Файл слишком большой. Попробуйте другой файл …';
	
	const SelectOption = 'Выбрать …';
	const SelectFilter = 'Фильтровать …';
...
namespace Localization\GameEditorPage\TooltipContent
{
	...
	const TransliteratedName = 'Романизация названия.<br/><br/>'.
	                           'Просим ознакомиться с правилами романизации в <a href="/ru/writing-guide#romanization" target="_blank">Руководстве</a>.<br/><br/>'.
							   'Отдельно стоит отметить то, что можно использовать только символы ASCII.<br/><br/>'.
							   '* = обязательно';
	const LocalizedName      = 'Название, адаптированное либо для международного рынка (обычно на английском языке).<br/><br/>'.
	...
	const NewLogo            = 'Если хотите заменить логотип, загрузите новый.<br/><br/>'.
	                           'Максимальный размер файла: 2 мегабайта.<br/><br/>'.
							   'Рекомендуется использовать изображение размером 384 на 384 пикселей.';
...
namespace Localization\ArtistEditorPage\TooltipContent
{
	...
	const TransliteratedName = 'Романизация имени.<br/><br/>'.
							   ...
							   'Отдельно стоит отметить то, что можно использовать только символы ASCII.<br/><br/>'.
							   '* = обязательно';
	const LocalizedName      = 'Имя, адаптированное либо для международного рынка (обычно для английского языка).<br/><br/>'.
...
namespace Localization\CharacterEditorPage\TooltipContent
{
	...
	const TransliteratedName = 'Романизация имени.<br/><br/>'.
							   ...
							   'Отдельно стоит отметить то, что можно использовать только символы ASCII.<br/><br/>'.
							   '* = обязательно';
	const LocalizedName      = 'Имя, адаптированное либо для международного рынка (обычно для английском языка).<br/><br/>'.
	...
	const NewImage           = 'Максимальный размер файла: 2 мегабайта.<br/><br/>'.
							   'Рекомендуется использовать изображение размером 384 на 384 пикселей.'.
...
namespace Localization\FillAlbumPage\TooltipContent
{
	const TrackTable = 'Таблица сформирована на основе данных, полученных из vgmdb.net.<br/></br>'.
					   ...
					   'Проверьте, что названия соответствуют правилам в <a href="/ru/writing-guide#romanization" target="_blank">Руководстве</a>.'.
...
namespace Localization\LyricsEditorPage\TooltipContent
{
	...
	const Language           = 'Укажите язык исполнения песни.<br/><br/>'.
							   ...
							   '* = обязательно';
	const Lyrics             = 'Напишите слова песни. Просим ознакомиться с правилами оформления в <a href="/ru/writing-guide#lyrics" target="_blank">Руководстве</a><br/><br/>'.
							   ...
							   'Подробности использования можно найти в <a href="/ru/writing-guide#formatting" target="_blank">Руководстве</a>.<br/><br/>'.
							   '* = обязательно';
...
namespace Localization\TranslationEditorPage\TooltipContent
{
	...
	const TranslationLyrics   = 'Переведите слова песни.<br/><br/>'.
	                            'Просим ознакомиться с рекомендациями в <a href="/ru/writing-guide#translation" target="_blank">Руководстве</a>.<br/><br/>'.
								'Здесь можно использовать специальный синтаксис, чтобы сделать текст более понятным.<br/><br/>'.
	                            '{kj}漢字{fg}かんじ{/fg}　→　<ruby>漢字<rt>かんじ</rt></ruby><br/><br/>'.
	                            '{cl #FF0000}色づいた{/cl}言葉　→　<span style="color: #FF0000">色づいた</span>言葉<br/><br/>'.
	                            '{nt}1{/nt}　→　Если указать строку такого вида и в словах, и в заметках, то между ними будут рабочие ссылки.<br/><br/>'.
...
namespace Localization\PolicyPage
{
	...
	const ContentPolicy1  = '1. Весь вклад пользователя является доступным для любого пользователя сети Интернет. Пользователь не может предъявлять права на любой вклад, созданный им или другим участником.';
	const ContentPolicy2  = '2. Наименования и переводы наименований игр и игровых персонажей, исполнителей и альбомов, а также все изображения собраны из разных источников и могут быть объектом прав их соответствующих правообладателей.';
	const ContentPolicy3  = '3. Тексты песен могут быть написаны с помощью покупных изданий музыки или записаны на слух.'.
	                        'Поэтому контент может походить на содержание других сайтов. Как бы то ни было, тексты принадлежат только правообладателям песен.';
	const ContentPolicy4  = '4. Пользователю не разрешается публиковать переводы, являющиеся плагиатом работ, опубликованных где-либо ещё.';
	const ContentPolicy5  = '5. Переводы песен являются общественным достоянием. Никто, не ограничиваясь в том числе администрацией сайта, не вправе предъявлять права на них.'.
	                        'Хотя из этого следует, что данный контент может быть использован кем угодно для любых целей, администрация просит вас:';
	const ContentPolicy5a = '5a. Если вы используете переводы, то убедительно просим указывать как ссылку на веб-сайт, так и создателя перевода.';
	const ContentPolicy5b = '5б. Просим не использовать контент для целей машинного обучения.';
	const ContentPolicy6  = '6. Любая сущность на сайте или её часть может быть убрана по требованию её правообладателя.';
	const ContentPolicy7  = '7. Сайт и его администрация не несут ответственности за возможный урон и другие негативные последствия, вызванные работой сайта или его содержанием. '.
	                        'Ошибки в работе сайта и злонамеренные действия третьей стороны также относятся к этому правилу.';
	const ContentPolicy8  = '8. Политика может быть изменена без уведомления пользователей.';
	...
	const HeadingContentWarning = 'Предупреждение о контенте';
	const ContentWarning1       = '1. Сайт стремится стать базой данной для всех песен и их переводов независимо от их содержания. '.
	                              'Всегда есть вероятность, что какой-либо контент может показаться оскорбительным.';
	const ContentWarning2       = '2. Сайт соблюдает законодательство Российской Федерации и не содержит материалов, нарушающих его. '.
	                              'Законодательство в других странах может отличаться: в таких случаях используйте сайт на свой страх и риск.';
	const ContentWarning3       = '3. Сайт собирает данные со всего мира, поэтому никакая их часть не представляет мнений администрации сайта.';
	
	const LastUpdated = 'Последнее изменение: 29 июня 2026.';
...
namespace Localization\RulesPage
{
	...
	const GeneralRule2  = '2. Имя пользователя и описание профиля не должно носить оскорбительный, неприличный или политический характер.';
	const GeneralRule3  = '3. Пользователю не разрешается публиковать контент, не связанный с тематикой сайта.';
	const GeneralRule4  = '4. Администрация сайта оставляет за собой право удалять любой контент, имеющий сомнительный или противоречивый характер.';
	const GeneralRule5a = '5. Администрация сайта оставляет за собой право редактировать и удалять любой контент, не следующий правилам ';
	const GeneralRule5b = 'Руководства';
	const GeneralRule5c = '.';
	const GeneralRule6  = '6. Любое нежелательное поведение, не попадающее под правила выше, может быть приравнено к нарушению.';
	const GeneralRule7  = '7. Администрация сайта оставляет за собой право ограничивать доступ к ресурсу полностью или только часть функционала любого пользователя без объяснения причин и уведомлений.';
	
	const HeadingPublication = 'Правила публикации';
	
	const PublicationRule1 = '1. К «играм» относятся только видеоигры жанра «визуальная новелла». Принадлежность жанру можно проверить с помощью сайта vndb.org.';
	const PublicationRule2 = '2. К «альбомам» относятся только официальные альбомы от издателей и разработчиков, а так же фанатские работы. Так называемые «gamerip» не разрешены.';
	const PublicationRule3 = '3. К «исполнителям» чаще всего относятся именно вокалисты, но в редких случаях может быть указана и вся музыкальная группа.';
	const PublicationRule4 = '4. К «персонажам» относятся только те персонажи игр, которые указаны как «исполнители» песни в титрах игры, буклетах альбомов и мета-тегах у музыкальных файлов. Чаще всего это обозначается как «Имя персонажа (CV. Имя вокалиста)».';
	...
	const LastUpdated = 'Последнее изменение: 01 августа 2026.';
	const Timezone    = 'Часовой пояс веб-сайта: UTC+3.';
}
...
namespace Localization\WritingGuidePage
{
	...
	const AllowedSymbols        = 'Романизация японского языка использует только те символы, которые присутствуют на обычной раскладке для английского языка.';
	...
	const WidthOfCharacters   = 'Ширина символов';
	const AsianLanguages      = 'Языки Азии: ';
	const EuropeanLanguages   = 'Языки Европы: ';
	const Punctuation         = 'Пунктуация';
